...
# https://github.com/khanalsaroj/typegen
############################################################
...
generate_config() {
  ...
  cat > "$config_file" <<EOF
# ⚡ TypeGen Configuration v${version}
...
setup_auto_completion() {
  info "⚡ Setting up auto-completion..."
...
[Unit]
Description=TypeGen Service
Documentation=https://.dev
...
main() {
  ...
  info "🚀 Initializing TypeGen Installer"
  ...
  echo ""
  echo "${COLOR_BOLD}✨ TypeGen  Installation Complete! ✨${COLOR_RESET}"
  echo ""
  echo "┌─────────────────────────────────────────────────────────┐"
  ...
  if [ -t 0 ]; then  # Check if stdin is a terminal
    cat <<EOF
      ...
      ${COLOR_SUCCESS}Thank you for using TypeGen!${COLOR_RESET}
EOF
  fi
